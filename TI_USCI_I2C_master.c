...
unsigned char RexBuf[maxlength];
extern volatile unsigned char startsend;
extern volatile unsigned char receiveFlag;


void TI_USCI_I2C_Init()
{
...
//P1OUT=0X02;
  ...
  {
    RexBuf[count] = UCA0RXBUF;
    if(flip==0)
    {
      if(RexBuf[count] == startnode)
      {
        flip = 1;
        count++;
      }
    ...
    if(count>=2)
        {
      if(RexBuf[count] == endnode)
          {    
        _BIC_SR_IRQ(LPM1_bits);
            if (RexBuf[count-1] == startchar)                     // 'u' received?
        {
              receiveFlag=1;
        }
          else if (RexBuf[count-1] == stopchar)                     // 's' received?
        {
            receiveFlag=2;
        }
